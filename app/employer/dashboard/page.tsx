    ...
    useEffect(() => {
        const fetchJobs = async () => {
            try {
                ...
                // Filter to get only active jobs (status "APPROVED")
                ...
                const totalApplications = data.jobs.reduce(
                    (acc: number, job: { totalApplications: number }) =>
                        acc + job.totalApplications,
                    0
                );

                // For each active job, fetch its applications.
                const jobsWithApplications = await Promise.all(
                    activeJobs.map(async (job: any) => {
                        const resApps = await fetch(`/api/jobs/${job.id}/applicants`);
                        const dataApps = await resApps.json();
                        return {
                            ...job,
                            applications: dataApps.applications, // attach the applications array
                        };
                    })
                );
                ...
                setStats({
                    ...
                    activePostings: activeJobs.length, // use the length here
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span
                                                    className={`px-2 py-1 text-sm rounded-full ${job.status === "PENDING"
